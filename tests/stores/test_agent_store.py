...
def test_get_session_scoped_agent_resolves_owning_session_not_child(
    agent_store: SqlAlchemyAgentStore,
    db_uri: str,
) -> None:
    """A session-scoped agent's session_id is its owning session, never a child.

    Regression for OMNI-1611: named ``sys_session_send`` children are created
    bound to the *same* agent_id as their parent, so multiple conversation rows
    share it. ``get()`` must resolve ``agent.session_id`` to the owning
    (top-level) session, not a child. The child row is inserted first *and*
    given a lexically smaller id than the owner, so the buggy unordered
    ``LIMIT 1`` — whether served by a table scan (rowid/insertion order) or the
    ``(workspace_id, agent_id, id)`` index (id order) — returns the child and
    fails the assertion. The fix's ``parent_conversation_id IS NULL`` filter
    returns the owner regardless of row order.
    """
    agent_id = "6ec5d35246127ba7b23bd47aa95208ec"
    owner_id = "ff2c8fc48e056223d18a47a8d4660491"
    child_id = "112c8fc48e056223d18a47a8d4660491"
    engine = get_or_create_engine(db_uri)
    with engine.begin() as conn:
        # Named child first (lower ts): shares the parent's agent_id and points
        # back at the owner via parent_conversation_id.
        conn.execute(
            sa.text(
                "INSERT INTO conversations "
                "(id, created_at, updated_at, root_conversation_id, "
                "agent_id, parent_conversation_id) "
                "VALUES (:id, :ts, :ts, :root, :agent, :parent)",
            ),
            {
                "id": bytes.fromhex(child_id),
                "ts": 1700000000,
                "root": bytes.fromhex(owner_id),
                "agent": bytes.fromhex(agent_id),
                "parent": bytes.fromhex(owner_id),
            },
        )
        # Owning top-level session (parent_conversation_id NULL).
        conn.execute(
            sa.text(
                "INSERT INTO conversations "
                "(id, created_at, updated_at, root_conversation_id, agent_id) "
                "VALUES (:id, :ts, :ts, :id, :agent)",
            ),
            {
                "id": bytes.fromhex(owner_id),
                "ts": 1700000001,
                "agent": bytes.fromhex(agent_id),
            },
        )
        conn.execute(
            ...
            ),
            {
                ...
                "ts": 1700000001,
                "name": "orchestrator",
                "loc": "ag_orchestrator/bundle",
            },
        )

    fetched = agent_store.get(agent_id)
    assert fetched is not None
    assert fetched.session_id == owner_id
